...
parser.add_argument('--num_epochs', type=int, default=200, help='max epochs')
...
def load_split_dfs(dataset_index: int):
    ...
    train_path = "/project/zhiwei/g013857/TCR-Multi/dataset/few_shot_split/pep+cdr3b"

    # /project/zhiwei/g013857/TCR-Multi/dataset/ds.hard-splits/pep+cdr3b/train/only-sampled-negs/train-0.csv
    # /project/zhiwei/g013857/TCR-Multi/dataset/few_shot_split/pep+cdr3b/train/only-sampled-negs/1-train-0.csv
    train_file = f'{train_path}/train/{mode}/{args.few_shot}-train-{dataset_index}.csv'
    ...
    # val_df = None
    val_file = f'{train_path}/validation/{mode}/{args.few_shot}-validation-{dataset_index}.csv'
    val_df = pd.read_csv(val_file, low_memory=False)
    val_df = val_df[~val_df[TCR_col].isin(values_to_remove)] # remove bad data entry

    test_file = f'{train_path}/test/{mode}/{args.few_shot}-test-{dataset_index}.csv'
